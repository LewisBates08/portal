"""Fictional browser-test data. Never copied to the production image."""
...
def seed_browser_fixture(password):
    settings = get_settings()
    from sqlalchemy.engine import make_url

    if settings.environment != "test" or not make_url(
        settings.database_url
    ).database.endswith("_test"):
        raise RuntimeError(
            "Browser fixtures require test mode and an isolated _test database."
        )
